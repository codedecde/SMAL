...
@BaseAcquisitionFunction.register("sequence_tagging_simple_token_entropy")
class SequenceTaggingSimpleTokenEntropy(BaseAcquisitionFunction):
    @overrides
    def score_instances(self, instances: List[Instance]) -> torch.Tensor:
        """Scores instances based on token entropy.
        ...
        with torch.no_grad():
            ...
            # Divide by the sequence length to normalize
            normalized_sequence_entropy = sequence_entropy / mask.float().sum(-1)
            # Higher entropy -> more uncertain. Hence we add a -ve,
            # and sort ascending later to collect more uncertain examples
            scores = -normalized_sequence_entropy
        return scores
